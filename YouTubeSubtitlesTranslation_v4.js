...
function composeDualXML(body, map, layout) {
  ...
  let result = body.replace(/<p\b([^>]*\bt="(\d+)"[^>]*)>([\s\S]*?)<\/p>/gi,
    (full, attrs, ms, content) => {
      const orig  = content.replace(/<[^>]+>/g, "").replace(/\n/g, " ").trim();
      if (!orig) return full;
      const trans = map[String(ms)] || fuzzyGet(map, parseInt(ms));
      if (!trans || trans === orig) return full;
      hit++;
      // 清空内部标签，用纯文本双行替换
      return "<p" + attrs + ">" + encodeHTML(makeLine(decodeHTML(orig), trans, layout)) + "</p>";
    }
  );

  // srv1/srv2：替换 <text start="..."> 内容
  if (hit === 0) {
